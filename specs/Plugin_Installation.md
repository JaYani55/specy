...
6. Rebuilds the generated plugin registries, API mount files, and wrangler binding section from the `plugins/` folder
...
If the plugin declares `wrangler_bindings` in its `plugin.json`, those bindings are automatically injected into `wrangler.jsonc` during the same rebuild step. The injection happens inside a dedicated auto-generated section marked by:
...
`r2_buckets`, `vars`, and `secrets_store_secrets` are owned by the CMS core and cannot be injected through plugins. The build system validates binding name uniqueness across all plugins and errors on conflicts.
