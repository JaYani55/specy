...
**Token acquisition paths:** dashboard users sign in with email/password (Supabase password grant). Programmatic MCP clients use OAuth 2.1 Authorization Code + PKCE against the Supabase authorization server, discovered via `GET /.well-known/oauth-protected-resource` (RFC 9728) — see [`OAuth_MCP_Authentication.md`](oauth-mcp-authentication.md). Both token types are validated identically below. All 401 responses include a `WWW-Authenticate` challenge.
