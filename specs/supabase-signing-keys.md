...
Start off by clicking the Migrate JWT secret button on the JWT signing keys page. This step will import the existing legacy JWT secret into the new JWT signing keys system.
...
This repository is now aligned to Supabase's signing-keys model.

- API auth validation in [api/lib/auth.ts](../api/lib/auth.ts) uses `supabase.auth.getClaims(token)` so asymmetric signing keys can be verified through Supabase JWKS discovery instead of a locally configured shared JWT secret.
- The current custom claims system remains centered on [migrations/Auth/Access_hook.sql](../migrations/Auth/Access_hook.sql), which injects `claims.user_roles` into every access token issued by Supabase Auth.
- Frontend role gating continues to read `user_roles` from the access token payload in [src/contexts/AuthContext.tsx](../src/contexts/AuthContext.tsx). This remains compatible with asymmetric signing keys because the frontend decodes claims but does not locally verify signatures.
- The managed `send_email` Edge Function must keep `verify_jwt = false` in both [functions/config.toml](../functions/config.toml) and [.supabase-deploy/supabase/config.toml](../.supabase-deploy/supabase/config.toml) because it is invoked server-to-server with the Supabase secret key rather than an end-user JWT.

## Project migration checklist

Use this sequence when rotating the hosted project away from the legacy JWT secret:

1. Confirm the deployed repo revision includes the signing-key-compatible auth helper in [api/lib/auth.ts](../api/lib/auth.ts) and the `verify_jwt = false` Edge Function config in [.supabase-deploy/supabase/config.toml](../.supabase-deploy/supabase/config.toml).
2. In Supabase Dashboard, open Authentication > JWT Signing Keys and click **Migrate JWT secret**.
3. Leave the imported legacy key in place and allow Supabase to create a new asymmetric standby key.
4. Verify the custom access token hook is still enabled and points to `pg-functions://postgres/public/custom_access_token_hook`.
5. Rotate to the standby asymmetric key.
6. Sign in with a test user and confirm the new access token still contains `user_roles` from the hook.
7. Validate one API route that uses role gating plus one RLS-backed content query.
8. Wait at least the access-token lifetime plus 15 minutes before revoking the previously used legacy JWT secret, unless an incident requires immediate revocation.
9. After all clients have moved to publishable/secret API keys, disable legacy JWT-based anon and service_role keys in the Supabase API Keys settings.

## Post-rotation validation

After rotating keys, verify all of the following with freshly issued tokens:

- login succeeds and frontend navigation still reflects `user_roles`
- API calls protected by `requireAppRole()` still authorize correctly
- RLS-backed queries still honor the caller's tenant and role visibility
- plugin and addon visibility that depends on `user_roles` continues to work
