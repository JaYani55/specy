...
-   **Plugin-Contributed Entity Actions (Hook-Driven Button Row):** The `src/components/entity-actions/EntityActionsRow.tsx` component is a generic, pluggable action-row that appears inside core editor pages (Schema Page Builder, Form Editor, Object Editor, and Event Edit). It collects UI actions from plugins through the `knowledgeBase.entity.actions` hook target on mount: it calls `getPluginHooks('knowledgeBase.entity.actions', user.roles)`, walks every registered hook handler in the `transform` contract, and renders small action buttons (e.g. "KB Sync") with a loading spinner and toast-based success/error feedback. If no plugin contributes actions, the component renders nothing — no plugin dependency is hardcoded.
-   **AutoSync on Create (Plugin Hook):** The `knowledgeBase.entity.afterCreate` hook target (`src/services/knowledgeBaseSyncService.ts`) is fired fire-and-forget by the core SchemaPageBuilderForm, FormEditor, ObjectEditor, and CreateEvent pages after a successful create operation. PluraDash registers a handler that checks `kb_enabled` and `kb_auto_sync_enabled` settings and triggers a synchronous KB document write if both are true. The hook scope is `ui` with `kind: observer`, meaning it never blocks or modifies the payload.
-   **Backend Sync Invocation (Generic API Gateway):** The core route `POST /api/knowledge-base/sync` (defined in `api/routes/knowledgeBase.ts`) accepts `{ sourceTable, sourceRecordId, tenantId }`, authenticates via `requireAuthSession`, then dispatches to the `knowledgeBase.sync.invoke` hook target. PluraDash implements the handler: it resolves the tenant, loads the source record with the caller's own JWT (RLS-respecting, no admin client), serializes the entity to markdown, writes it to R2, upserts the `knowledge_base_documents` row, and rebuilds chunk rows. If no plugin handles the hook, the endpoint returns 501. This keeps the core entirely plugin-agnostic while giving the KB feature a user-facing HTTP boundary. See [api/lib/knowledgeBaseHooks.ts](api/lib/knowledgeBaseHooks.ts) for the full hook context contracts and [plugins/pluradash/api/storageHooks.ts](plugins/pluradash/api/storageHooks.ts) for the PluraDash implementations.
-   **AuthContext Lifecycle Hardening:** Refactored `AuthProvider` to eliminate a critical async race condition between the `onAuthStateChange` listener and the manual `login()` function. The `isInitialAuthRef` flag is now explicitly set to `false` after the initial session check completes, preventing the listener from intercepting `SIGNED_IN` events triggered by manual logins. Added proper `subscription.unsubscribe()` cleanup in the `useEffect` return to fix a memory leak. Simplified `loadAppData` by removing an unnecessary `new Promise` wrapper around the synchronous `queryClient.setQueryData` call. Removed unreachable `window.location.reload()` call in `logout` (the preceding `window.location.href` assignment causes immediate navigation).
